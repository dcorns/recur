...
const rl = readline.createInterface({
  ...
  rl.question('Log N?', (n) => {
    ...
    console.log(`log N of ${n} = ${lg} Test accuracy 2^lg: ${2 ** lg}`);
    rl.close();
  });
...
function log2OfN(n:number, seed:number){
  console.log(`2 ** seed: ${2 ** seed}, n: ${n}, seed: ${seed}`);
  if (2 ** seed >= n) return seed;
  else log2OfN(n, seed + 1); //note that seed++ will always be 1 at the input because it is incremented after being calling the function ++seed will give the desired result of seed + 1 because it will be incremented first.
}
...
function log2OfNReturnRecursive(n:number, seed:number){
  console.log(`2 ** seed: ${2 ** seed}, n: ${n}, seed: ${seed}`);
  if(2**seed >= n) return seed;
  else return log2OfNReturnRecursive(n, seed + 1);
}
...
function log2(n:number, seed:number){
  console.log(`2 ** seed: ${2 ** seed}, n: ${n}, seed: ${seed}`);
  if(2**seed < n) return log2(n, seed + .0001);
  else return seed;
}
...
function log2a(a:number, b:number){
  function recursiveLog2(n:number, seed:number){
    console.log(`2 ** seed: ${2 ** seed}, n: ${n}, seed: ${seed}`);
    if(2**seed < n) return log2a(n, seed + .0001);
    else return seed;
  }
...
function logLoop(n:number, seed:number){
  while(2 ** seed < n){
    seed = seed + .0001;
  }
  return seed;
